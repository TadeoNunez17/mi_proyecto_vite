...
const BusinessNavTab = ({ currentRowInBusinessTab, setCurrentNameTabInBusinessTab }) => {
    ...
    const handleChange = (e) => {
        ...
        switch (e.target.innerText.toUpperCase()) {
            case "NEGOCIOS":       setCurrentTabIndex(0); break;
            case "INFO ADICIONAL": setCurrentTabIndex(1); break;
            case "ARCHIVOS":       setCurrentTabIndex(2); break;
            case "TELEFONOS":      setCurrentTabIndex(3); break;
            case "DIR WEBS":       setCurrentTabIndex(4); break;
            case "DOMICILIOS":     setCurrentTabIndex(5); break;
        }
    };
        ...
        <Box sx={{ border: (theme) => `1px solid ${theme.palette.divider}`, mx: 1, padding: 0.5 }}>
            ...
            >
                {BusinessTabs.map((tab) => (
                    <Tab key={tab} label={tab} disabled={currentRowInBusinessTab === null} />
                ))}
            </Tabs>
        </Box>
    );
